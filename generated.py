...
#下面一共是三个函数（下面的一些路径我测试过了，都是没问题，代码什么我也是单独的测试过了已经生成了所有的文件）
# encode_images    (提取你想要的图片的特征 ，提取训练集，验证集，测试集等等)
# build_train_vocab(只生成train的词库，之前我学习应该都是只用了train的，而且这里也只是将词库jieba划分,至于word转id，和id转word都是下面的build_samples的事情)
# build_samples     (生成train,valid的图片与标注数据)
...
#
#就是下面这行代码，弄得整个py成了一个pytest 测试框架
# def test_gen():
#     encode_images('test_a')
